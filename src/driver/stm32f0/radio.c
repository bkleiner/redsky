...
void radio_init() {
  ...
  gpio_config(CC2500_CSN_PIN, GPIO_OUTPUT | GPIO_PP);
  gpio_config(CC2500_SCK_PIN, GPIO_OUTPUT | GPIO_PP | GPIO_AF);
  gpio_config(CC2500_MISO_PIN, GPIO_INPUT | GPIO_OD | GPIO_AF);
  gpio_config(CC2500_MOSI_PIN, GPIO_OUTPUT | GPIO_PP | GPIO_AF);
  ...
  gpio_config(CC2500_GDO2_PIN, GPIO_INPUT | GPIO_OD);
...
static uint8_t packet_size() {
  /*
  if (gpio_read(CC2500_GDO2_PIN) == 0) {
    return 0;
  }
  */
